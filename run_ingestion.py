...
BTC-USD, authenticated via a CDP API key -- this restores full top-10 order
book depth, which the earlier public "ticker" channel fallback couldn't
provide (Coinbase's old public level2 channel now requires auth; Advanced
Trade's level2 channel requires the same, but is reachable with a free,
regular-account API key -- see jwt_auth.py).
...
Requires environment variables:
  ...
  COINBASE_API_PRIVATE_KEY_PATH

JWTs expire after ~120s, so the connection is periodically re-subscribed
with a fresh token well before expiry.
...
JWT_REFRESH_INTERVAL_SEC = 90  # tokens expire at 120s, refresh comfortably before that
        ...
        try:
            ...
            async for raw in ws:
                ...
                if now_check - last_jwt_refresh > JWT_REFRESH_INTERVAL_SEC:
                    ...
                    log.info("Refreshed JWT and resubscribed")
