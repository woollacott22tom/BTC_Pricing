...
BTC-USD.
  ...
  Root cause of the long debugging saga to get here: the "level2" channel's
  initial snapshot message can exceed the websockets library's default 1MB
  max message size, causing a silent, instant connection kill (code 1006,
  no error frame) the moment that snapshot arrived -- regardless of channel
  name, auth, or headers. Fixed here via max_size=None on the connection.
    ...
    while True:
        try:
            async with websockets.connect(
                ...
                max_size=None,  # level2's full order book snapshot can exceed the
                                 # default 1MB limit -- without this, the connection
                                 # gets silently killed the instant that snapshot arrives
            ) as ws:
                backoff = backoff_start
                yield ws
