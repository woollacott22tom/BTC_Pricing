...
from compute import (
    compute_block_summary, build_lookback_features,
    compute_continuation_labels, TIER_NAMES, AGG_FIELDS,
)
...
def build_block_dataset() -> pd.DataFrame:
    ...
    # No confirmation gate anymore -- every block gets a feature row.
    # Chunking (wait for a confirmed 3+ streak) required retroactive
    # knowledge and both leaked hindsight into training and went stale in
    # live serving. streak_length and the fixed trailing lookback need no
    # confirmation at all; 0 and 1 are valid, meaningful values here, not
    # excluded, per explicit decision not to bias toward streak
...
def train_continuation_model(df: pd.DataFrame):
    ...
    exclude_cols = {"window_id", "direction", "label_continuation"}
