...
def extract_prices(market: dict) -> dict | None:
    """Handles minor field-naming variance seen across Kalshi API examples
    (cents as int vs. a dollars field) defensively -- logs a warning rather
    than silently writing garbage if the shape is unexpected."""
    yes_bid = market.get("yes_bid")
    yes_ask = market.get("yes_ask")

    if yes_bid is None or yes_ask is None:
        log.warning(f"Unexpected market response shape, missing yes_bid/yes_ask: {list(market.keys())}")
        return None

    # Kalshi's documented format is cents (1-99); guard against an
    # unexpected dollars-format value (e.g. 0.62) slipping through.
    if 0 < yes_bid < 1:
        yes_bid *= 100
    if 0 < yes_ask < 1:
        yes_ask *= 100
    ...
    return {
        ...
        "yes_mid_cents": float(yes_bid + yes_ask) / 2.0,
